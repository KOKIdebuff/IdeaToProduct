...
def validate_bundle(
    ...
    *,
    ...
    The import stays lazy so Registry/resolution code remains usable without
    initializing the repository validator and so direct script execution keeps
    the same public resolver surface.
    """
    context = bundle if isinstance(bundle, BundleContext) else load_bundle_context(bundle)
    try:
        from scripts.validate_contracts import validate_bundle as validate_selected_bundle
    except ModuleNotFoundError:  # direct execution from the scripts directory
        from validate_contracts import validate_bundle as validate_selected_bundle
