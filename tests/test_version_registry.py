...
def test_registry_schema_and_config_are_valid():
    ...
    assert registry.default_new_run_version == "0.2.0"
    assert set(registry.versions) == {"0.1.0", "0.2.0"}
    assert registry.versions["0.1.0"].status == "immutable_baseline"
    assert registry.versions["0.2.0"].status == "current"
...
def test_registry_compatibility_matrix_is_default_deny_and_read_only():
    ...
    for rule in registry.compatibility_rules:
        ...
        assert rule.target_contract_version == "0.2.0"
...
def test_bundle_resolution_defaults_new_runs_to_v02_and_keeps_v01_audit_only():
    registry = load_version_registry(require_bundles=False)

    current = resolve_contract_bundle(registry=registry)
    assert current.contract_version == "0.2.0"
...
def test_versioned_references_must_match_the_selected_bundle():
    assert require_bundle_version("developer_tool@0.2.0", "0.2.0") == "developer_tool"

    with pytest.raises(ContractResolutionError) as captured:
        require_bundle_version("developer_tool@0.1.0", "0.2.0")
    assert captured.value.code == "SCHEMA_VERSION_UNSUPPORTED"
...
def test_v02_bundle_inventory_is_complete_and_v01_integrity_is_locked():
    registry = load_version_registry()
    current = resolve_contract_bundle(registry=registry)
    context = load_bundle_context(current)

    assert len(list(context.schema_dir.glob("*.schema.json"))) == 23
    assert len([path for path in context.skill_dir.iterdir() if path.is_dir()]) == 23
    assert len(list(context.profile_dir.glob("*.yaml"))) == 4
    assert len(list(context.subgraph_dir.glob("*.yaml"))) == 1
    assert len(list(context.template_dir.glob("*.md"))) == 5
    assert verify_integrity(registry) == []


def test_public_bundle_validator_keeps_v01_and_v02_contexts_isolated():
    registry = load_version_registry()
    baseline = resolve_contract_bundle("0.1.0", operation="audit", registry=registry)
    current = resolve_contract_bundle("0.2.0", operation="new_run", registry=registry)
